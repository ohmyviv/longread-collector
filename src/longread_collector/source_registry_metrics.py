...
def update_source_registry_metrics(
    ...
    *,
    ...
    """Update operational source-registry fields after a directed scan.

    The current counters are cumulative shadow diagnostics. A later scheduled
    maintenance task may replace them with true rolling-30-day aggregates.
    """

    attempted = {source_id for source_id in attempted_source_ids if source_id}
    if not attempted:
        return
    ...
    for row_number, row in enumerate(rows[1:], start=2):
        ...
        if source_id not in attempted:
            continue
        existing_discovered = _as_int(row[19] if len(row) > 19 else 0)
        existing_extracted = _as_int(row[20] if len(row) > 20 else 0)
        worksheet.update(
