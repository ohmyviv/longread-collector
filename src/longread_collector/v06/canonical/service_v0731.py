"""PR-7.3.1 Canonical Article wrapper for date-provenance hotfixes.

PR-7.3 source relationships and PR-7.1 publication safety remain the baseline.
This wrapper changes only the publication-evidence bridge/resolution path; L5
Editorial Judge semantics remain frozen at PR-7.2.
"""
...
CANONICAL_SERVICE_VERSION = "canonical-article-resolver-v0.6-pr7.3.1"


class CanonicalArticleResolver(_PR71CanonicalArticleResolver):
    """Resolve PR-7.3 source facts plus PR-7.3.1 date provenance."""
    ...
    def canonicalize(
        ...
        # The evidence-authoritative PR-7.3 rule remains unchanged: if the
        # richer resolver cannot establish an initial publication fact, keep it
        # unknown rather than reviving a demoted URL/republication projection.
...
def _strip_demoted_url_date_projections(
    ...
    """Remove generic adapter copies of a known URL-derived legacy fact.

    Structured PR-7.3.1 publication evidence is stored separately on
    ``bundle.evidence`` and is deliberately not removed here.  Therefore an
    independent page-metadata date on the same calendar day as the URL survives
    URL demotion and remains selectable by L4.
    """
