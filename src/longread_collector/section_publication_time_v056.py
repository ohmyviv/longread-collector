...
observations. This module enriches only explicitly supported first-party section
surfaces when the page itself exposes an unambiguous relative calendar-day clock
(`今天 HH:MM` / `昨天 HH:MM`) adjacent to an article card.

It deliberately does not infer years from `MM-DD`, convert vague values such as
`40分钟前`, inspect article bodies, or assign nominal route windows. Unsupported or
ambiguous evidence remains date-unknown.
"""
...
SECTION_PUBLICATION_TIME_VERSION = "section-publication-time-observability-v0.1"
...
def _relative_day_clock(
    ...
    *,
    ...
    """Resolve only explicit 今天/昨天 clock evidence against page observation day."""
...
def enrich_section_publication_times(
    ...
    *,
    ...
    """Attach page-surface publication time only where evidence is explicit."""
    ...
    for item in items:
        ...
        if evidence is None:
            continue
        published_at, raw = evidence
        item.published_at = published_at.strftime("%Y-%m-%d %H:%M:%S")
        item.metadata.update(
            {
                "published_at_source": "section_relative_day_clock",
...
def install_section_publication_time_observability() -> None:
    """Patch the v0.5.6 section parser without changing discovery selection."""
