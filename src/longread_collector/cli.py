...
@app.command()
def doctor(test_remote: bool = typer.Option(False, help="Also make one Jina and one Firecrawl request")) -> None:
    """Validate credentials, Sheet schema, query groups, and optionally remote APIs."""
    ...
    if test_remote:
        ...
        try:
            ...
            report["jina"] = {"ok": bool(data.get("markdown")), "chars": len(str(data.get("markdown", "")))}
        except Exception as exc:
            report["jina"] = {"ok": False, "error": f"{type(exc).__name__}: {exc}"}
        try:
            results, meta = asyncio.run(firecrawl.search("long-form journalism", limit=1, tbs="qdr:d"))
            report["firecrawl"] = {"ok": True, "results": len(results), "credits_used": meta.get("credits_used")}
        except Exception as exc:
            report["firecrawl"] = {"ok": False, "error": f"{type(exc).__name__}: {exc}"}
    typer.echo(json.dumps(report, ensure_ascii=False, indent=2))
    if not report["sheet"]["ok"] or any(v == 0 for v in report["groups"].values()):
...
def extract(url: str, language: str = "") -> None:
    ...
    firecrawl = FirecrawlClient(settings.firecrawl_base_url, settings.firecrawl_api_key)
    jina = JinaReaderClient(settings.jina_reader_base_url, settings.jina_api_key)
    article = asyncio.run(extract_article(DiscoveredURL(url=url, language=language), jina, firecrawl, settings))
