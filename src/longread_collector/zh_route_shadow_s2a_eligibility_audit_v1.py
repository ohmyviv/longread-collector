...
Shadow item telemetry plus timestamp-measurement-v2. It never performs
Discovery, network requests, body extraction, Sheet writes, Editor wiring, or
production mutation.

The deterministic part is cohort construction and reviewed-label validation.
Metadata eligibility labels remain explicit reviewed judgments; this module
deliberately does not pretend title-only classification is objective truth.
"""
...
S2A_ALLOWED_CLASSES = frozenset(
    {
        "plausible_standard_longread",
...
def build_s2a_cohort(rows: Iterable[dict[str, Any]]) -> tuple[S2ACohortItem, ...]:
    """Build the strict canonical-unique S2-A cohort.

    Inclusion requires Jiemian-depth/Yicai, v2 `fresh`, a non-noise-control
    surface, explicit same-run `control_overlap=FALSE`, and canonical identity.
    Duplicate rows across runs/surfaces collapse while provenance is retained.
    """

    grouped: dict[str, dict[str, Any]] = {}
    for row in rows:
        source_id = _text(row.get("source_id"))
        if source_id not in S2A_SOURCES:
            continue
        if _text(row.get("surface_role")) == "noise_control":
            continue
        ...
        run_id = _text(row.get("collector_run_id"))
        ...
        if existing is None:
            grouped[url] = {
                ...
                "title": title,
                "row_count": 1,
                "surfaces": {surface_id} if surface_id else set(),
                "runs": {run_id} if run_id else set(),
            }
            continue
        ...
        if surface_id:
            existing["surfaces"].add(surface_id)
        if run_id:
    ...
    items = [
        S2ACohortItem(
            ...
            title=value["title"],
            qualifying_row_count=int(value["row_count"]),
            qualifying_surfaces=tuple(sorted(value["surfaces"])),
            collector_run_ids=tuple(sorted(value["runs"])),
        )
        for url, value in grouped.items()
    ]
    return tuple(sorted(items, key=lambda item: (item.source_id, item.url_canonical)))


def validate_reviewed_labels(
    cohort: Iterable[S2ACohortItem],
    labels: Mapping[str, Mapping[str, str]],
) -> dict[str, Any]:
    """Fail closed if the human-reviewed metadata ledger does not match cohort."""

    items = tuple(cohort)
    cohort_by_url = {item.url_canonical: item for item in items}
    cohort_urls = set(cohort_by_url)
    label_urls = set(labels)
    missing = sorted(cohort_urls - label_urls)
    unexpected = sorted(label_urls - cohort_urls)
    ...
    for url in sorted(cohort_urls & label_urls):
        ...
        if metadata_class not in S2A_ALLOWED_CLASSES or reason not in S2A_ALLOWED_REASONS:
            ...
            continue
        by_class[metadata_class] += 1
        by_reason[reason] += 1
        by_source[cohort_by_url[url].source_id][metadata_class] += 1

    return {
        "version": S2A_ELIGIBILITY_VERSION,
        "valid": not missing and not unexpected and not invalid,
        "cohort_total": len(items),
        "reviewed_total": len(cohort_urls & label_urls),
