...
1. **Firecrawl Search**按14个开放主题查询发现新URL，并主动排除当前最集中的核心域名；
2. **Jina Reader**优先把公开网页转换为Markdown；
3. Jina正文不足时，**Firecrawl Scrape**作为回退，但全系统每天最多调用3次；
4. 规范URL、跨查询去重、自动估计正文访问验证等级；
5. 将缓存、解析日志、运行状态写入现有Google Sheet；
6. 先以 `shadow` 模式运行，达到健康阈值后再让07:35日报缓存优先。
...
达到后采集器会自动把 `collector_config.mode` 从 `shadow` 切换为 `cache_primary`，日报随后按安全门执行：

```text
article_cache优先 → 原生搜索补缺 → 最终原文复核
```

自动切换要求连续72小时内12次成功轮换、过去48小时至少18篇可编辑文章、至少12个域名且解析成功率不少于60%。即使切换后健康门暂时失效，日报也会自动回退原生检索。在此之前，原日报完全按现有流程运行，不会因为采集器未部署或失败而中断。
